...
Summarizes the results of the upstream scripts which have:
(1) gathered exomes meeting the specified copyng criteria
(2) extracted and modified crispr sites and produced a new set of fastas
with only this data.
...
def main():
    ...
    if not os.path.exists(args.clinical_txt):
        print(f"clinical file {args.clinical_txt} not found.  Exiting...")
        sys.exit(1)
    if not os.path.exists(args.exome_dir):
        print(f"exome directory {args.exome_dir} not found.  Exiting...")
        sys.exit(1)
